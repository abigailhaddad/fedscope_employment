...
format:
  html:
    output-file: forest_service.html
    toc: true
    toc-depth: 3
    code-fold: true
...
# Configuration
LOCAL_PARQUET_DIR = "../../employment_cube/parquet/"

# Forest Service agency code
FOREST_SERVICE_CODE = "AG11"  # Using the code, not the name

# Define time periods to analyze (now including all periods from parquet files)
TIME_PERIODS = [
    ...
    {"year": 2024, "quarter": "September"},
    {"year": 2025, "quarter": "March"},
]
```
...
def load_historical_data(year: int, quarter: str) -> pd.DataFrame:
    ...
    file_path = f"{LOCAL_PARQUET_DIR}{filename}"
...
def load_summary_data(datecode: int, year: int, quarter: str) -> pd.DataFrame:
    ...
    file_path = Path("../../employment_cube/status_reports_march_2025/Status Employment by Agency and SubAgency_202503_and_202409.txt")
...
# Get appointment types over time
appointment_data = get_appointment_types(all_data)

if not appointment_data.empty:
    # Load the full appointment type descriptions from summary data
    appointment_file = Path("../../employment_cube/status_reports_march_2025/Status Employment by Appointment Type_202503_and_202409.txt")
